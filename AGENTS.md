...
> **Maintenance rule (for Codex):** update this file at the end of every work session — refresh the
...
Journal features in **APK 1.6.0** — see the Android repo's AGENTS.md. **Web not yet committed/deployed**
...
three Journal fixes — see that repo's AGENTS.md. **Web not yet committed/deployed** (awaiting the usual
...
bar); see the Android repo's AGENTS.md. Prior 2026-07-28: **New module — Tadabbur**, a reel-style, swipeable stream of the Qur'an
...
destination + `android-tadabbur.css` overlay — see the Android repo's AGENTS.md and the Last-changes
...
1.4.1** (see the Android repo's AGENTS.md): Read Mushaf no longer double-draws a header on the phone,
...
- Local preview: `.Codex/launch.json` has "root" (port 5501, serves repo root) and "search"
...
- **2026-08-03 — Web source for APK 1.7.0: shared reader-defaults listeners + Mushaf font-key unification.**
  ...
  expanded About — rebuilt + V2-signed (SHA-1 `d81d0f12…`), archived. See the Android repo's AGENTS.md.
...
- **2026-07-29 — Tadabbur buttons no longer cover the ayah text.** User report: in the Tadabbur reel the
  ...
  repo's AGENTS.md. **Web change is not yet committed or deployed** — commit and `npx vercel --prod` when ready.
...
- **2026-07-29 — CSV export works in the Android app + Urdu on the occurrence cards (web + APK 1.4.6).**
  ...
  **Android — SHIPPED as APK 1.4.6** (versionCode 17, versionName 1.4.6; see the Android repo's AGENTS.md):
...
- **2026-07-28 — New module: Tadabbur (web only).** A reel-style, swipeable stream of the Qur'an built
  ...
  - **Android — SHIPPED as APK 1.4.3** (code 14; see the Android repo's AGENTS.md). The user clarified
...
- **2026-07-24** **Root→book audit finished across the whole 1,664-root list** (the task the ص ر ط
  ...
  **p009.webp was rendered** — the only new image). App-root coverage **1,620 → 1,655 / 1,664**. The
  ...
  to Android and shipped as signed APK 1.3.4** (versionCode 8) — see the Android repo's AGENTS.md.
...
- **2026-07-24** **The book index now finds the root ص ر ط (ṣ-r-ṭ, "path" — as in ٱلصِّرَٰطَ
  ...
  **Synced to Android and shipped as APK 1.3.3** (the Android native `root_directory.json`, built by
  ...
  same way); that APK also carries the new launcher icon (see the Android repo's AGENTS.md).
...
- **2026-07-24** **Two reader fixes, both shared with the Android app (they reach it through
  ...
  AGENTS.md (WebView opaque bg + hardware layer for scroll; drawer opens only from ☰; notes
...
- **2026-07-10** Added "Pairs →" button to every ayah card in Explore Quran → deep-links to
  Explore Connections at that ayah (`../?ayah=SN:AN`). Created this AGENTS.md context file.
